...
impl SymbolFetcherTrait for SymbolFetcher {
  async fn refresh(&self) -> SendableErrorResult<()> {
    ...
    let old_symbols: Vec<String> = ret_on_err!(col.find(doc! {}, None).await)
    ...
    if resp_status.is_success() {
      ...
      let serialized: Vec<Document> = ret_on_err!(to_bson(&info.symbols))
        ...
        .collect();
      let (ins_res, _) =
        join(col.insert_many(serialized.into_iter(), None), update_event).await;
      ret_on_err!(ins_res);
      return Ok(());
    } else {
